...
@section('content')
    <x-dashboard.header>
        ...
        <div class="text-xl font-bold">{{ __('dashboard.edit_faq') }}</div>
    </x-dashboard.header>
    ...
    <div class="bg-white rounded-lg shadow p-6 h-full w-full md:w-2/3">
        ...
        <form action="{{ route('dashboard.faqs.update', $faq->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="question_ar" name="question_ar" value="{{ $faq->question_ar }}"
            ...
            <div class="mb-4">
                ...
                <input type="text" id="question_en" name="question_en" value="{{ $faq->question_en }}"
                    class="mt-1 w-full rounded-md bg-gray-100 px-2 py-1 hover:shadow outline-none focus:shadow">
                @error('question_en')
                    <span class="text-red-500">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                <label for="answer_ar"
                    class="block text-sm font-medium text-gray-700">{{ __('userarea.answer_ar') }}</label>
                <textarea id="answer_ar" name="answer_ar" rows="3"
                    class="mt-1 w-full rounded-md bg-gray-100 px-2 py-1 hover:shadow outline-none focus:shadow">{{ $faq->answer_ar }}</textarea>
            </div>
            <div class="mb-4">
                <label for="answer_en"
                    class="block text-sm font-medium text-gray-700">{{ __('userarea.answer_en') }}</label>
                <textarea id="answer_en" name="answer_en" rows="3"
                    class="mt-1 w-full rounded-md bg-gray-100 px-2 py-1 hover:shadow outline-none focus:shadow">{{ $faq->answer_en }}</textarea>
            </div>
            <div class="mb-4">
                <button type="submit"
