...
@section('content')
    <div class="h-screen md:grid grid-cols-3">
        <div class="p-5 flex items-center justify-center bg-[#472A12]">
            <form method="post" action="{{ route('register.check_otp') }}" class="w-full">
                @csrf
                <p class="text-2xl font-bold mb-10 text-white"><i class="fa-solid fa-lock ml-2"></i>{{ __('dashboard.check_otp') }}</p>

                <div class="mb-4" id="otp_div">
                    <label for="otp" class="block mb-1 text-white ">{{ __('dashboard.otp') }}</label>
                    <input type="otp" id="otp" name="otp" class="w-full p-2 border bg-gray-100 rounded-lg">
                    @error('otp')
