...
@section('content')
    <x-dashboard.header>
        <div class="text-xl font-bold">{{ __('dashboard.facilities') }}</div>
    </x-dashboard.header>
            ...
            <table class="shadow w-full rounded-lg overflow-hidden">
                <thead>
                    <tr class="bg-gray-200">
                        ...
                        <th class="py-2 px-4 text-center">{{ __('dashboard.category') }}</th>
                        <th class="py-2 px-4 text-center">{{ __('dashboard.type') }}</th>
                ...
                <tbody id="tableData">
                    @foreach ($facilities as $fa)
                        <tr class="border-t border-gray-200">
                            <td id="name_{{ $fa->id }}" class="py-2 px-4 text-start">{{ $fa->name_en . ' / ' . $fa->name_ar }}</td>
                            <td  class="py-2 px-4 text-start">
                                {{ $fa->category->name_en . ' / ' . $fa->category->name_ar }} </td>
                            <td class="py-2 px-4 text-center">
                                {{ __('dashboard.facilities_of_' . $fa->type) }}
                            </td>
                            <td class="py-2 px-4 text-start">
                                <div>
                                    <button onclick="openModal(this)"
                                    data-modal-id="editName"
                                    data-name-ar="{{ $fa->name_ar }}"
                                    data-name-en="{{ $fa->name_en }}"
                                    data-id="{{ $fa->id }}"
...
<script>
    ...
    function submitEditName(id) {
        let route = "{{ route('dashboard.facilities.update') }}";
